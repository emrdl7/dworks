...
| 사용자 결정 | (2026-05-09) | "ai생성쪽은 시작도 못하는 이유가 뭐야?" — M2 트랙 mandate 흡수 완료, M3 AI 생성 트랙 진입 + 가속 모드 유지 |
| `m3-generate-mvp` | r1 (Claude `e4d6e00`) → r2 (Codex 조건부 동의 `e5915e4`) → 사용자 ack → r4 (Claude API + prompt + adapter `24d5517`) → r5 (Codex test 5건 `28d8f38`) → r6 (Claude web UI `eb60396`) | POST /generate (Hono) + Claude CLI 어댑터 (spawn 주입, 30s timeout) + GENERATE_TREE_SYSTEM_PROMPT (8 노드 종류 schema 가이드 + hero root 예시) + treeSchema 검증 후 full Tree 반환. 에러 코드 400 (invalid)/422 (parse·schema)/502 (cli). web 헤더에 "AI 생성" 영역 (시각 분리 divider+강조 라벨), prompt 입력 + Enter/버튼 → fetch /generate → commitTreeEdit (undo 자동 push), 실패 시 캔버스 유지 + status text. 라운드 4/5/6 분할 |
| 사용자 결정 | (2026-05-09) | "한 줄 prompt 빈약. 좌측 사이드 + 추가 정보 수집 + 생성 후 트리 토글" — 헤더 한 줄 입력 → 좌측 패널 + 5필드 brief 전환 (m3-generate-brief) |
| `m3-generate-brief` | r1 (Claude `1db76bf`) → r2 (Codex 조건부 동의 `91aa5ef`) → r4 (Claude `fd5aa88`) | 헤더 한 줄 prompt 제거 + 좌측 "AI 디자인" disclosure (의도/페이지타입/톤/섹션/메모 5필드) + 생성 히스토리 chip group (max 6, immutable 원본 baseline + mutable generations) + commitTreeEdit 통합 + handleSelectGeneration 토글. API request body { brief: { intent, pageType?, tones?, sections?, notes? } } primary, { prompt } legacy compat. **m3-generate-clarify에서 5필드 UI 완전 교체** — brief는 stepping stone |
| 사용자 결정 | (2026-05-09) | "첫 의도를 입력받고 의도에 맞는 질문이나 선택지를 생성해서 보이는게 어때?" — Stitch / ChatGPT 플러그인 패턴, 고정 5필드 → LLM 적응형 질문 |
| `m3-generate-clarify` | r1 (Claude `9234e14`) → r2 (Codex 조건부 동의 `dc30057`) → r4 (Claude `d6d84cc`) → r5 (Codex test 12건 통합 `e873a6d`) | POST /clarify 신규 (intent → questions 3~6, single/multi/text + options 2~6). brief 재정의 (answers + questionId/Label, pageType/tones/sections 제거). CLARIFY_QUESTIONS_SYSTEM_PROMPT (도메인별 axis 가이드). web 5필드 완전 교체 — Stage 1 (intent + "질문 받기" / "의도만으로 바로 생성" fallback) + Stage 2 (의도 read-only + "수정" + 적응형 질문 카드 + 추가 메모 + "디자인 생성"). resetBriefForm 성공 시 자동, 히스토리 토글 규칙 유지. brief r5 본 토픽 r5에 흡수 |
| `m3-generate-fallback` | r1 (Claude `5c07a26`) → r2 (Codex 조건부 동의 `d87eb6d`) → r4 (Claude `eb6aada`) → r5 (Codex test 21건 `c312aba`) | LlmProvider enum + resolveProviderChain (DWORKS_LLM_PROVIDERS env, trim/dedupe/unknown 무시, 빈 chain 시 502 cli-unavailable). callLlmCli(provider, options) 일반화 + callClaudeCli backward compat. callLlmChain({ chain, ... }) 순회 — 첫 성공 즉시 반환, 모두 실패 시 마지막 attempt. parse/schema 실패는 fallback 안 함 (Codex r2). Codex / Gemini는 stub spawn (envelope 정밀 매핑 후속). response.model에 실제 응답 provider 노출. /generate + /clarify 모두 chain 적용 |
| `m3-generate-codex-adapter` | r1 (Claude `4345825`) → r2 (Codex 동의 `2d6edc8`) → r4 (Claude `9f61575`) → r5 (Codex test 25건 `b40cd6a`) | Codex provider invocation에 `-o <tempfile>` 추가 (judge.ts callCodex 패턴). runProvider async — codex일 때 mkdtemp + outputFile 주입, spawn close 후 fsReadFile, finally cleanup. allowEmptyStdout 옵션으로 stdout-vs-file 경로 분리. ClaudeCliFailureKind에 'extract-failure' 추가 — outputFile 누락/empty/read 실패 → 다음 provider 시도. parse/schema 실패는 fallback 없음 (경계 명확). LlmCliOptions / Generate·ClarifyDeps에 readOutputFileImpl + prepareOutputFileImpl 주입 (테스트). 기존 fallback 테스트 코덱 stdout → 주입 패턴으로 변경 |
| `m3-generate-variant` | r1 (Claude `d78c666`) → r2 (Codex 동의 `0c7ef85`) → r4 (Claude `9b6b503`) → r5 (Codex fix + web test `d31e3f2`) | 같은 brief로 N (1/2/3) 변형 동시 생성. server contract 변경 0 — web에서 Promise.allSettled로 /generate N parallel. VARIANT_DIVERSITY_HINTS 3종 (안정형/임팩트형/차분 실무형)을 brief.notes에 request-only append (저장 brief는 원본 유지). 부분 성공 inline status `${count}개 중 ${success}개 생성됨`. Codex fix: appendGenerationHistoryEntries 헬퍼 추출 — max trim이 immutable 원본을 밀어내지 않게 보호. web 테스트 인프라 도입 |
| `m3-generate-prompt-uplift` | r1 (Claude `5f3656a`) → r2 (Codex 동의 `d132cfa`) → r4 (Claude `f6eba0e`) → r5 (Codex test 강화 `2280a56`) | GENERATE_TREE_EXAMPLES 3 schema-valid Tree (카페 / SaaS 가격제 / 블로그 글) + CLARIFY_QUESTIONS_EXAMPLE 1 도메인 4문항 (도시 호텔 — action/content/target/voice). TS 객체 + JSON.stringify로 prompt 삽입, llm-prompts test에서 treeSchema / clarify response schema로 drift 방지. 스타일 prop 1차 제외 (emphasis/variant/aspectRatio만), prose 절제로 토큰 비용 완화 |
| `m3-generate-eval` | r1 (Claude `e57b6f4`) → r2 (Codex 동의 `530b680`) → r4 (Claude `639ed03`) → r5 (Codex fix `6dcfe6f`) | 신규 apps/m3-eval CLI — fixtures × repeat × providers 호출, status (ok/invalid-request/parse-failure/schema-failure/cli-*/transport-error) + requestLatencyMs (HTTP RTT) + modelLatencyMs (API body) 분리, treeNodeCount/Depth, manifest/calls.jsonl/summary.{json,md} 저장. dry-run 기본 + --live 명시. seeds/m3-eval/intents.json 8 도메인. Codex fix: /generate body에 request-level providers override + repo root 기준 fixture 경로 해석 + unknown provider 거부 |
| `m3-generate-clarify-edit` | r1 (Claude `d1328fa`) → r2 (Codex 동의 `d47dbc3`) → r4 (Claude `d456089`) → r5 (Codex fix `ab2a52a`) | active variant 선택 시 그 entry의 brief(intent/answers/notes) + clarify questions snapshot을 Stage 2 폼에 복원, 답변 일부 수정 후 "디자인 생성"으로 새 변형 추가. GenerationEntry에 questions: ClarifyQuestionDto[] 필수 필드 추가 (briefSchema 변경 0, client-only). target.questions ?? [] 방어 + 배열 답변은 [...answer] deep copy. 원본(brief: null) 선택 시 Stage 1 reset + generateError clear. intent-only entry(questions.length === 0)는 Stage 1 복원. handleGenerateTree 성공 후 resetBriefForm 제거 → 폼이 firstNew와 자동 일치 (dead code 정리). Stage 2에 "선택한 디자인의 답변" 라벨 (variant active일 때만). Codex fix: buildSubmittedBrief에서 generateStage === 'intent'면 intent만 반환 — Stage 1에서 "의도만으로 바로 생성" 시 화면에 안 보이는 stale answers가 brief에 섞이는 문제 차단 |
| `m3-generate-prompt-style` | r1 (Claude `93b156a`) → r2 (Codex 동의+필드명 정정 `2ad7620`) → r4 (Claude `f98a6e2`) | GENERATE_TREE_EXAMPLES 3개에 노드별 스타일 시그니처 — 카페 hero(backgroundGradient 베이지→오렌지 + spacing padding/gap + shape.radius + button radius 32 shadow sm + image radius), SaaS 가격(section/plans layout direction + gap, pro card backgroundColor accent + radius + shadow lg + textColor 흰색, starter/enterprise borderWidth+borderColor 절제), 블로그 매거진(section padding+gap+layout column, intro/body typography.lineHeight 1.7, cover image radius). examples block 223 → 298 라인 (1.34×, 1.6× 가드 내). schema 변경 0, web 변경 0. system prompt 정책 완화 — "스타일 prop 생략" → "도메인 시그니처 명확하면 최소 범위로 사용, 과한 장식 피하고 구조/콘텐츠 우선, transition/transform/cursor/responsive/상태 색은 명시 요청 없으면 생략". drift 방지 test 3건 — DFS로 각 example에 ≥ 2 카테고리 + 3개 통합 color/spacing/shape 커버 + 금지 필드명 substring (`"gradient":`/`"borderRadius":`/`"shadow":{"preset"`/`"color":{"color":`/`"padding":`) 0건. Codex round 2 흡수: schema 필드명 정정(backgroundGradient/textColor/paddingTop/Right/Bottom/Left/spacing.gap/shape.radius/shape.shadow=preset enum), raw hex 허용 + example 색 2-4개 제한, typography는 lineHeight만 |
| `m3-generate-eval-diversity` | r1 (Claude `e2d36e1`) → r2 (Codex 동의+multiset 보정 `29f204f`) → r4 (Claude `1de372e`) → r5 (Codex fix `4b98f10`) | m3-eval에 같은 intent의 repeat 성공 결과들 사이 pairwise 트리 구조 유사도(type-path multiset Jaccard) 측정 → summary intent table에 `구조 다양성` 컬럼 추가. diversityScore = 1 - avgPairwiseSimilarity, 트리 < 2이면 null. apps/api/packages/tree/llm-prompts/web 변경 0. 신규 apps/m3-eval/src/diversity.ts (computeBag/jaccard/pairwiseSimilarity/computeDiversity). run.ts: successfulTreesByIntent in-memory(status 'ok' + treeSchema 통과 tree만), calls.jsonl에는 tree 안 넣음. summary.ts: IntentSummary.diversityScore + 별도 INTENT_TABLE_HEADER, null=`-`, 값=`0.00` 2자리 fixed. Codex round 2 흡수: multiset Jaccard(intersection=sum of min, union=sum of max)로 반복 type-path 차이 반영, type-path > type만(root 맥락 보존), 컬럼명 "구조 다양성"(콘텐츠/스타일 보장 안 함 신호), diversityScore = 1 - avgSimilarity만 1차. Codex fix: dry-run 메모를 `!live && repeat >= 2` 조건으로 강화(repeat=1은 다양성 측정 불가) |
| `m3-generate-variant-grid` | r1 (Claude `ef341b9`) → r2 (Codex 동의+read-only 강화 `42193a6`) → r4 (Claude `892290f`) → r5 (Codex fix `fc8223a`) | compareMode 토글 → 캔버스 영역에 mini canvas grid (모든 generations) → cell 클릭/Enter/Space → 그 entry active + compareMode off + 단일 캔버스 복귀. 비교는 read-only view, 편집은 늘 단일 모드. 신규 CanvasReadOnlyContext (top-level createContext) + VariantCompareGrid + MiniGenerationCanvas (page.tsx 하단). SelectableNode이 useContext로 read-only 시 role/tabIndex/data-dworks-node-id/toolbar/selected border 모두 제거된 단순 div 렌더 (onSelect noop만으로 부족). cell은 div role="button" tabIndex={disabled?-1:0} aria-label="{label} 선택하여 편집" aria-current. inspector hidden + main grid 2열, chip group disabled+aria-disabled, context menu compareMode 시 early return, scale = MINI_CELL_WIDTH(320)/selectedViewportPreset.width로 viewport 정합. apps/api/packages 변경 0. Codex round 2 흡수: generations.length >= 2 활성 (원본 + 1개도 비교), helper 같은 파일 하단 (분리는 후속), viewport 강제 X, copy "변형 비교"/"비교할 디자인을 선택하면 편집 화면으로 돌아갑니다.". Codex fix: fixture 전환 + 생성 성공 후 firstNew active 시 compareMode false로 자동 복귀 + generateLoading 중 cell 선택/focus 차단(disabled prop) |
| `m3-generate-clarify-loop` | r1 (Claude `341b19b`) → r2 (Codex 동의+schema 분리 `26e89fa`) → r4 (Claude `763b657`) → r5 (Codex fix `4565e81`) | 선택형 multi-turn clarify. 첫 turn 답변 후 "더 구체적으로 답변하기" → /clarify history 첨부 재호출 → LLM이 부족 영역 추가 질문 1~3개 또는 빈 배열(충분) 반환. max 2 history turns. API: clarifyTurnSchema(questions 1~6 + answers 0~6) + request.history(max 2) + response 분리(clarifyResponseFirstSchema 3~6 / clarifyResponseFollowUpSchema 0~3) — handleClarify가 history.length 보고 분기. buildClarifyUserPrompt가 history를 user prompt에 직렬화 + 이전 id 재사용 금지 + 충분 시 빈 배열 지시. system prompt에 "Multi-turn" 섹션. web: ClarifyTurn interface, GenerationEntry.clarifyTurns(snapshot, briefSchema 변경 0), clarifyComplete state, collectCurrentAnswers 헬퍼, buildSubmittedBrief 평탄화(questionId Map 최신 우선 + label도 최신 + max 6 slice), handleAskFollowUp(completed turn push + history 재호출 + 빈 응답이면 complete), 의도 박스 다음 이전 turn collapsed cards(details), 답변 폼 다음 "답변이 충분합니다" 신호 또는 "더 구체적으로 답변하기" 버튼(turns < 2 + 현재 답변 ≥ 1). Codex round 2 흡수: 첫 turn 빈 배열 422, follow-up max 3 schema, GenerationEntry.clarifyTurns snapshot, questionId 중복 최신 우선, 이전 turn 카드 collapsed 기본, max 2. Codex fix: 충분 상태로 끝난 entry 복원 시 clarifyComplete = (turns > 0 && questions empty) + Stage 2 유지. 별도 fix `c984b21` — api에 hono/cors 추가(web cross-origin fetch dev 환경 정합) |
| `m3-generate-prompt-emotional-mapping` | r1 (Claude `258a212`) → r2 (Codex 동의+팔레트 추상화 `9a60285`) → r4 (Claude `7b5230a`) | system prompt에 "## 답변 → 스타일 매핑 가이드" 14줄 섹션 추가 — 5종 톤(차분/절제/신뢰, 활기/임팩트/강조, 친근/따뜻/부드러움, 전문/정확/깔끔, 프리미엄/고급)을 색 계열(cool muted / warm accent / 흑백 / deep premium) + spacing.padding 범위 + shape.shadow preset enum + typography.lineHeight + layout.direction에 매핑. raw hex 강제 X(LLM 다양성 보존), 복합 톤은 주 톤 1개 우선 + 보조 톤 1~2개 prop만 약하게. 디자인 브리프 처리 다음 examples 직전 위치. drift 방지 test 강화 — header + 5개 톤 라벨 모두 검증(본문 회귀 차단). 위치: examples / schema / web / api 변경 0 |
| `m3-generate-prompt-page-foundations` | r1 (Claude `fe01ae3`) → r2 (Codex 동의+role 정합 `7e367cf`) → r4 (Claude `1ab5273`) → r5 (Codex fix `9aee05a`) | 사용자 정정 흡수 — 첫 시안에 헤더/푸터 부재, 반응형 미적용, 디자인 감각 결여. WebSearch 2026 트렌드 + references/claude-design-system-prompt 자산 활용. system prompt에 "## 페이지 기본 구조"(root section + banner/main/contentinfo 3 children, 헤더 nav contentRole='label', responsive 의도 메모) + "## 2026 디자인 트렌드 가이드"(bold typography hero / ample whitespace padding≥80 gap≥48 / single primary CTA / neutral base + accent 1~2 / mobile-first column 우선 + AI 슬롭 회피 4종) 두 섹션 추가. examples 3개 풀 페이지로 재구성: cafe.page / saas.page / magazine.page — 각 root section + banner(로고+nav contentRole='label') + main(기존 도메인 시그니처 보존) + contentinfo(카피라이트 + 보조 링크 2~3개). responsive 의도 2건 시범(cafe.header + cafe.main hero). examples block 298 → ~600 라인(2× — Codex round 2 합의). schema 정합 정정: layout.justify enum 'between'(not 'space-between') 4 군데. drift test 2건 신규: 페이지 기본 구조 + 2026 트렌드 헤더 + 모든 example의 root section + banner/main/contentinfo role 모두 존재. Codex fix: 절대 규칙의 모호 입력 fallback을 단편 hero에서 풀 페이지 기본 구조로 정정. schema/api/web 변경 0. 후속 분리: m3-generate-responsive-rendering(web viewport별 layout 자동 적응), m3-generate-eval-page-shell(vision LLM judge), m3-generate-nav-list(헤더 nav 1급 list 노드), m3-generate-eval-ai-slop(AI 슬롭 자동 detection) |
| `m3-generate-responsive-rendering` | r1 (Claude `b2d3510`) → r2 (Codex 동의+보정 5건 `cf8cd26`) → 코드 (`5bb1de6` Claude r4) → Codex r5 추가조치 없음 명시 | viewport별 layout 자동 적응. 신규 `apps/web/src/app/responsive.ts` (resolveResponsiveSpacing/resolveResponsiveLayout helper) + `responsive.test.ts` 11 unit. CanvasViewportContext 도입(main + mini canvas 같은 viewport 공유). mobile 규칙: explicit `direction='row'` → column, layout override 있으나 direction 없는 structure/hero container도 column, `layoutIntent='grid'` wrapper 1열, hero default split grid 1열, padding ≥32→24/≥80→60, gap ≥32→20. tablet: padding ≥32→28만. desktop: 기존 동작 유지. spacing 축소는 원본 값 존재 시에만 (기본 padding inline override 안 함). schema/api/llm-prompts 변경 0 |
| `m3-generate-mobile-nav-collapse` | 코드 (`df4c564` Claude 직접 r4) | 모바일/태블릿 헤더 햄버거(`MobileBannerSection`). desktop 기존 BannerSection 유지, mobile은 토글 버튼 + 펼침 list 렌더. CanvasViewportContext === 'mobile' 또는 'tablet' 분기. role/aria-expanded/aria-controls 키보드 정합. 가속 §6 round 1 docs 생략 |
| `m3-generate-prompt-visual-richness` | 코드 (`39d015c` 1차 / `93f413c` 2차 SaaS+블로그 / `667b3f0` 카페 gallery, 모두 Claude 직접) | 와이어프레임 탈피 + 시각 풍부도 강화. system prompt에 typography.fontSize 강제 사용 지침 + 색감/shadow 가이드 보강. 1차: 카페 hero 색감 강화, SaaS 가격 카드 절제 강조. 2차: SaaS 추가 섹션, 블로그 매거진 typography 보강 + drift test 강화. gallery: 카페 example에 menu+gallery 섹션 추가(grid 4 카드). examples block 추가 누적(이전 ~600 → 약 750+). 가속 §6 round 1 docs 생략 |
| `m3-generate-schema-retry` | 코드 (`665a428` Claude 직접) | 422 (parse-failure / schema-failure) 시 1회 자동 재시도. handleGenerate에 runOnce inner async fn + while loop. DWORKS_GENERATE_RETRIES env override (default 1, NODE_ENV=test=0). retry 시 DWORKS_DEBUG_SCHEMA=1이면 attempt 로그. cli-* 에러는 retry 안 함(network/spawn 단계는 fallback chain 책임) |
| `m3-generate-prompt-strict-schema` | 코드 (`6de3a1c` Claude 직접) | system prompt에 "Schema 정확한 형식" 절대 규칙 섹션. backgroundColor/textColor hex 6자리만/letterSpacing -0.1~0.2 ratio number/justify enum 'between'-'evenly'-'start'-'center'-'end'/role enum 'banner'-'main'-'contentinfo' 명시. schema-resilience 1차 시도(prompt-side). LLM이 prompt 지침을 매번 따르지 않을 가능성 인지 → schema-resilience 토픽으로 이어짐 |
| `m3-generate-schema-resilience` | r1 (Claude 핸드오프 `668df18`) → r4 (Codex 직접 sanitize layer `b49e439`) | Codex 주도 토픽 (사용자 명시 "코덱스한테 해결하라고 넘겨"). 신규 `apps/api/src/sanitize.ts` — sanitizeGeneratedTreeJson 재귀 정규화. 처리 케이스: rgba/rgb/hsl 색→hex(+opacity peer 자동 채움 backgroundOpacity/textOpacity/borderOpacity/accentOpacity/overlayOpacity/fromOpacity/toOpacity/opacity), letterSpacing string '-0.02em'/number 범위 외→clamp(-0.1, 0.2), fontWeight 'bold'/numeric 950→100단위 round + clamp 100~900 string, justify 'flex-start'/'space-between' alias→enum, role 'header'/'footer' alias→'banner'/'contentinfo'. treeSchema 자체는 엄격 유지(원본 schema 변경 0), /generate 응답만 sanitize 후 검증. DEBUG_SCHEMA log에 raw + sanitized head 함께 출력. sanitize.test.ts 2건(정규화 + clamp) + generate.test.ts 보강. 35/35 test 통과. apps/api/typecheck + build + apps/tree typecheck + test + llm-prompts test 모두 통과 |
| `m3-generate-persist` | 코드 (`bae7c89` Claude 직접 r4) | 사용자 정정 흡수 ("생성 결과물이 휘발되어버리는 현상 해결해"). 페이지 새로고침/탭 닫기 시 generations + brief + clarify state가 component state로만 유지되어 100% 휘발하던 문제. localStorage 단일 키 `dworks:m3-generate:v1` (version 1 schema). 신규 `apps/web/src/app/generation-storage.ts` — readPersistedGenerationState/write/clear helper. treeSchema.safeParse로 저장된 tree 재검증, JSON 파싱 실패+activeGenerationId 누락+answer 비-string 등 schema 위반은 거부 후 손상된 키 삭제. SSR 환경(window 미정의)에서 read=null/write=no-op. QuotaExceeded 시 mutable generations만 제거하고 immutable + brief는 유지하는 1회 retry. page.tsx hydratedRef + 두 useEffect: 마운트 1회 복원(active generation의 tree로 setTree + selectedNodeId/historyPast/historyFuture 동기화), 11개 watched state(fixtureId/generations/activeGenerationId/brief 5필드/clarify 3필드) 변경 시 동기화. hydrate 전 write skip해 default 상태가 저장값을 덮어쓰지 않게 보호. fixture 토글은 기존대로 명시적 reset(handleFixtureChange는 변경 0). 신규 generation-storage.test.ts 9건 — round-trip / 빈 storage null / corrupt JSON 키 삭제 / version 불일치 / Tree schema 위반 거부 / activeGenerationId 누락 거부 / clear / SSR no-op / 비-string answer 거부 / clarifyTurn round-trip. typecheck + lint(0) + test 30/30 + build 통과. 가속 §6 round 1 docs 생략 |
| 사이드 fix (mandate 외 환경 정합) | 코드 (`05a1d79` page-shell 정리 / `be5ad22` LLM timeout 90s / `5540585` DWORKS_DEBUG_SCHEMA 진단 / `c984b21` hono/cors — clarify-loop 항목과 중복) | `05a1d79`: page-shell section default padding/gap 제거 + tablet 햄버거 적용. `be5ad22`: LLM 기본 timeout 30s → 90s, DWORKS_LLM_TIMEOUT_MS env override. `5540585`: schema-failure 시 server console에 issues + raw json head 진단 출력 (DWORKS_DEBUG_SCHEMA=1) |
